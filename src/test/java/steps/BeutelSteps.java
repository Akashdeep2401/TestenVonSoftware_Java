...
public class BeutelSteps {
    ...
    public void sind_nur_froschsteine_im_beutel_die_blau_grün_rot_oder_weiß_sind() {
        ...
        assertThat(container.logicUnderTest.getSpielBeutel().getFroschsteine()).allSatisfy(TestStein -> {assertThat(TestStein.getFroschsteinFarbe()).isIn(RichtigeFarben);});
    }
    ...
    public void sind_nur_froschsteine_mit_den_farben_der_spieler_im_beutel() {
        assertThat(container.logicUnderTest.getSpielBeutel().getFroschsteine()).allSatisfy(TestStein -> {assertThat(TestStein.getFroschsteinFarbe()).isIn(container.bereitsAusgewählteFarben);});
    }

    @Dann("sind {int} Froschsteine von jeder ausgewählten Farbe im Beutel")
    public void sind_froschsteine_von_jeder_ausgewählten_farbe_im_beutel(Integer AnzahlSteine) { // AnzahlSteine ist immer 10
        for (Color Farbe : container.logicUnderTest.players()) { // Für jede Farbe die ausgewählt wurde
            Condition<Froschstein> RichtigerStein = new Condition<>(TestStein -> TestStein.getFroschsteinFarbe() == Farbe, "Richtiger Stein"); // Ist die Froschsteinfarbe die Farbe eines Spielers
